...
try {
  ...
  const mascot = await call("/assets/cbu-sleeping-owl-v1.png");
  assert.equal(mascot.status, 200);
  assert.equal(mascot.headers["content-type"], "image/png");
  assert.match(mascot.headers["cache-control"], /immutable/);
