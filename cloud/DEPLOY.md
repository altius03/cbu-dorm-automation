...
백그라운드 실행은 비밀번호를 저장해야만 가능하므로 사용하지 않습니다. 긴 신청은 가능한 날짜를
최대 7박 8일의 연속 기간으로 묶어 요청 수를 줄입니다.
...
퇴관일은 사용자 데이터로 저장하지 않고 `overnight_private.residency_schedules`에서 제공합니다.
새 학기 공지가 나오면 아래 명령으로 갱신합니다.
