...
2. 전용 프로젝트 관리자 권한으로 `supabase/migrations/`의 SQL을 시간순으로 실행합니다. `overnight_app`의 로그인
...
화면의 `학교 내역으로 결과 다시 확인`은 저장 요청 없이 현재 학교 신청내역만 읽어 `saved`,
`overlap`, `not_attempted`로 대조합니다.
...
퇴관일은 사용자 데이터가 아닌 `overnight_private.residency_schedules` 운영 설정에서 제공합니다.
새 학기 모집 공지가 나오면 운영자가 서버용 DB 환경변수를 설정한 터미널에서 아래 명령으로 갱신합니다.
재배포는 필요 없으며 화면에 학기·갱신일·출처가 함께 표시됩니다. 일정이 없는 학기는 추정하지 않습니다.

```sh
npm --prefix cloud run schedule:set -- \
  --term 2027-1 --from 2027-02-14 --through 2027-08-28 \
  --semester 2027-06-23 --six-months 2027-08-15 --twelve-months 2028-02-13 \
  --source "2027학년도 생활관 모집 공지"
```

요청 로그는 계정·날짜·본문을 남기지 않고 API 종류, 전체 시간, 학교 로그인·내역 조회·DB 등
단계별 밀리초만 남깁니다. Workflow도 처리 기간 수와 학교 세션 준비 시간만 기록합니다.
